...
ù    ) -> Tuple[List[bytes], List[int], List[str]]:
    ...
    def to_torch_uint8(self, chunks: List[bytes]) -> torch.Tensor:
        if not chunks:
            return torch.empty((0, self.chunk_size), dtype=torch.uint8)
        arr = np.frombuffer(b"".join(chunks), dtype=np.uint8)
        N = len(chunks)
        return torch.from_numpy(arr).reshape(N, self.chunk_size).to(torch.uint8)
    ...
    def build_raw_dataset(
        ...
        # X = self.to_torch_uint8(chunks) # Converte la lista di chunk in un array NumPy uint8
        X = self.to_torch_uint8(chunks) # Converte la lista di chunk in un tensore uint8
        # y = np.array(labels, dtype=np.int64) 
        y = torch.tensor(labels, dtype=np.int64) # Converte la lista di etichette in un array NumPy
